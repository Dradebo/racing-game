...
function Recorder({ onComplete }: { onComplete: (samples: LapTraceSample[], durationMs: number, checkpointAtMs?: number) => void }) {
  ...
  useEffect(() => {
    ...
    const finish = () => {
      if (startAt.current === null || !recording) return
  ...
  useFrame(() => {
    if (!recording || startAt.current === null || !chassisBody.current) return
  ...
  return (
    <main style={{ position: 'fixed', inset: 0, background: '#05070a', color: 'white' }}>
      <div style={{ position: 'absolute', zIndex: 20, left: 12, top: 12, maxWidth: 360, background: 'rgba(0,0,0,.72)', padding: 12, borderRadius: 10 }}>
        ...
        {saved && <b style={{ fontSize: 12 }}>Lap saved. Return to normal Racecraft view.</b>}
      </div>
      <CanvasShell dpr={dpr} shadows={shadows} onStart={() => { actions.onStart(); window.dispatchEvent(new Event('racecraft-capture:start')) }} onCheckpoint={() => { actions.onCheckpoint(); window.dispatchEvent(new Event('racecraft-capture:checkpoint')) }} onFinish={() => { actions.onFinish(); window.dispatchEvent(new Event('racecraft-capture:finish')) }} onComplete={complete} />
      <Keyboard />
    </main>
  )
}

function CanvasShell({ dpr, shadows, onStart, onCheckpoint, onFinish, onComplete }: { dpr: number; shadows: boolean; onStart: () => void; onCheckpoint: () => void; onFinish: () => void; onComplete: (samples: LapTraceSample[], durationMs: number, checkpointAtMs?: number) => void }) {
